...
import ecdsa
from ecdsa import SigningKey, VerifyingKey, NIST384p, SECP256k1
...
class Blockchain:
    ...
    def __init__(self):
        ...
        self.private_key = sk_string.hex()
        vk = sk.verifying_key
        vk_string = vk.to_string()        
        self.public_key = vk_string.hex()
        self.public_addr = hashlib.sha256(str(self.public_key).encode()).hexdigest() #public addr
        """        
        self.transactions = []
    ...
    def broadcast(self, post_data):
        ...
        print("data to be sent:"+json.dumps(post_data))
        not_sent = 0
        network = self.nodes
        for node in network:
            print("sending post req to:"+f'http://{node}/add_broadcast_trans')
            response = requests.post(f'http://{node}/add_broadcast_trans', 
                                     json = post_data)
            print(f'http://{node}/add_broadast_trans - {response.status_code}')
...
def add_transaction():
    ...
    #print("sender" + rcvd_json['sender'])
    #print("sender1:" + blockchain.public_addr)
